def generate_price_series(symbol: str, period: str, frequency: str) -> pd.Series:
    ...
    # Tüm varlıkların paylaşacağı ortak faktörler
    # Seed sabit olduğu için aynı tarih aralığında aynı market/sector şokları oluşur.
    ...
    # Her sembol için ayrı idiosyncratic bileşen
...
_DEFAULT_PARAMS = {"drift": 0.00020, "idio": 0.0080, "mkt": 0.80, "tech": 0.00, "rates": 0.00, "gold": 0.00, "crypto": 0.00}from __future__ import annotations
...
_SYMBOL_PARAMS = {
    "AAPL": (0.00040, 0.0120), "MSFT": (0.00045, 0.0110),
    "GOOGL": (0.00035, 0.0130), "AMZN": (0.00030, 0.0155),
    "TSLA": (0.00025, 0.0310), "NVDA": (0.00065, 0.0250),
    "META": (0.00042, 0.0180), "JPM": (0.00028, 0.0100),
    "SPY": (0.00033, 0.0075), "QQQ": (0.00038, 0.0095),
    "GLD": (0.00010, 0.0068), "TLT": (-0.00008, 0.0065),
    "BTC-USD": (0.00055, 0.0360), "ETH-USD": (0.00048, 0.0420),
    "SOL-USD": (0.00060, 0.0480), "XAU": (0.00010, 0.0070),
}
_DEFAULT_PARAMS = (0.00020, 0.0180)
...
def generate_price_series(symbol: str, period: str, frequency: str) -> pd.Series:
    ...
    drift, vol = _SYMBOL_PARAMS.get(sym, _DEFAULT_PARAMS)
    bdays = pd.bdate_range(start=date.today() - timedelta(days=_PERIOD_DAYS[period]), end=date.today())
    prices = 100.0 * np.exp(np.cumsum(rng.normal(loc=drift, scale=vol, size=len(bdays))))
    daily = pd.Series(prices, index=bdays, name=sym)
    if frequency == "daily":
        return daily
    return daily.resample(_FREQ_ALIAS[frequency]).last().dropna()
...
def to_returns(prices: pd.Series, return_type: str) -> pd.Series:
    if return_type == "price": return prices
    if return_type == "log": return np.log(prices / prices.shift(1)).dropna()
...
def compute_statistics(returns_a: pd.Series, returns_b: pd.Series, frequency: str):
    ...
    stat = {
        "pearson_r": round(float(pr), 4), "pearson_p_value": round(float(pp), 6),
        "spearman_rho": round(float(sr), 4), "spearman_p_value": round(float(sp), 6),
        "r_squared": round(float(pr**2), 4), "observations": len(a),
        "is_significant": bool(pp < 0.05), "pearson_label": _label(pr),
...
def build_scatter_points(returns_a, returns_b, max_points=300):
    ...
    if len(df) > max_points: df = df.sample(max_points, random_state=42)
...
def _label(r):
    a = abs(r); d = "positive" if r>=0 else "negative"
    if a>=0.70: return f"strong {d}"
    elif a>=0.40: return f"moderate {d}"
    elif a>=0.20: return f"low {d}"
    return "very weak"
